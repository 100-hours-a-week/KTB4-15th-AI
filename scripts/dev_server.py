...
추천 모듈이 아직 비어 있으므로(NotImplementedError) 여기서만 가짜 결과를 꽂는다.
`app/` 안에는 가짜 데이터를 두지 않는다. Dockerfile 도 이 디렉터리를 COPY 하지 않는다.
...
if __name__ == "__main__":
    ...
    print(f"[dev] 추천은 가짜 데이터입니다. 인증 {'꺼짐' if settings.AUTH_DISABLED else '켜짐'}")
